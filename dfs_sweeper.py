class DFSSweeper(object):
    ...
    def move(self, cur, dir, callback_fn):
        ...
        if not self.visited(right):
            ...
            turn_taken += 1
            if self.robot.move():
                self.move(right, (dir + 1) % 4, callback_fn)
        ...
        if not self.visited(down):
            for _ in range(2 - turn_taken):
                ...
                turn_taken += 1
            if self.robot.move():
                self.move(down, (dir + 2) % 4, callback_fn)
        ...
        if not self.visited(left):
            for _ in range(3 - turn_taken):
                ...
                turn_taken += 1
            if self.robot.move():
                self.move(left, (dir + 3) % 4, callback_fn)
